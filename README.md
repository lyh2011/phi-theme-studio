...
- 导出可直接解压到 `resources/html/b19/themes/` 的主题 ZIP，样式表可选「覆盖模式」或「自包含模式」
...
2. 切换结果预览，在画布中选择或拖动语义元素，通过右侧“样式”面板编辑布局、尺寸、颜色与外观；“外观 > 背景”可为成绩卡、玩家昵称背景框等任意元素上传图片或设置渐变、圆角和透明度。需要完全跟随指针微调时按住 `Shift` 拖动，也可用方向键逐像素移动。
...
在「导出」面板可以选择 `b19.css` 的两种形态，两者在 phi-plugin 中渲染结果一致，区别只在于基础样式从哪里来。
...
五种预览共用同一套主题 CSS；`studio.json` 记录了所选形态与可编辑配置，phi-plugin 会忽略该文件。
