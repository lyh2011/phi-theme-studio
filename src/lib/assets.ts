...
export function safeAssetPath(path: string) {
  ...
  if (/^[a-z][a-z\d+.-]*:/i.test(path) || path.includes('?') || path.includes('#') || path.includes('%')) return false
...
export async function assetFromFile(file: File, path: string): Promise<PackageAsset> {
  const bytes = new Uint8Array(await file.arrayBuffer())
  return hydrateAsset({ path, mime: file.type || mimeFromPath(path), bytes })
}

export function hydrateAsset(asset: Omit<PackageAsset, 'previewUrl'>): PackageAsset {
